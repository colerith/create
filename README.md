...
-   **整点简报 (`broadcast`)**:
    -   每小时用一张短卡片汇总新帖、作品更新、每日推荐和频道统计，空时段不打扰。
...
- **时间**：北京时间整点汇总上一完整小时，例如 22:00 播报 `[21:00, 22:00)`。
  启动后等待下一个整点，不补发停机期间的历史简报。
- **新帖**：复用论坛统计缓存，合并实时论坛列表与新帖事件；只包含指定服务器中
  `@everyone` 和机器人均可查看的论坛。独立保存新帖事件，补齐统计扫描前就归档的新帖。
- **更新**：复用附件保护系统已发布的更新日志；只选目标服务器、同样可公开查看的作品。
  同一作品所在帖子一小时内多次更新合并为一条，链接指向最新更新消息。
  不会把普通聊天、旧作品的顶帖或未发布的草稿算成更新。
- **长度**：所有内容合并到一条消息，新帖和更新各列最多 3 条，超过时显示剩余数量。
  有已登记的公开日报面板时，附上完整日报入口。
- **文案**：沿用其他功能的奇米语（奇米蛋、小喇叭、作品蛋、小篮子、捏～），
  按「只有新帖 / 只有更新 / 两者都有」轮换简短开场，数量来自真实汇总，不生成虚构内容简介。
- **日推**：每天第一条非空简报从现有日推候选池（角色卡、预设、美化、工具、小剧场、世界书，
  排除置顶帖）附带 1 条推荐，此后当天不再重复附带。候选池为空时不消耗当天次数。
  该精选独立于各频道每日推荐面板的随机结果。日额度按简报发送整点的北京时间日期计算。
- **统计**：附一行分区数量、缓存收录帖子数和累计赞/评论；明确标为缓存统计，
  不将累计值当作一小时新增值。没有新帖或更新时，日推和统计也不单独触发消息。
...
隐藏分区、私密帖子不进入简报；若社区论坛仅对认证身份组开放，当前公开范围规则会将其排除。
